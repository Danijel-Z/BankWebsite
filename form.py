...
class TransferForm(FlaskForm):
    ...
    choice = RadioField('Choice option', validators=[DataRequired(message='Choose pay or transfer')], choices=[('Deposit'), ('Withdrawal'), ('Transfer'), ])
    amount = IntegerField('Amount', validators=[DataRequired()])
    submit = SubmitField()
    
    def validate_amount(form, amount):
        choosenAccount = Account.query.filter(Account.id == int(form.fromAccount.data)).first()
        if (form.choice.data == "Transfer" and amount.data >  choosenAccount.Balance):
...
class Deposit_Withdrawal_Form(FlaskForm):
    ...
    amount = IntegerField('Amount', validators=[DataRequired(), NumberRange( min= 0, max= 15000, message="Max amount $15,000") ])
    submit = SubmitField()
